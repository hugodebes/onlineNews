...
def global_info():
  ...
  st.markdown(
    ...
    provided by the _UCI Machine Learning Repository_. This dataset is part of a research project aiming at predicting the popularity of an article on one hand and optimizing the features
...
def subjectivity_influence():
  ...
  img= load_image(url)
  st.image(img)
  st.markdown("""As we can see above, we have a **switch** around 0.4 from negative values to a positive ones. It means that there are more successful articles than unsuccessful
...
def author_influence():
  st.header("Does your name really matter ? 🕵🏽‍♂️")
...
def overview():
  ...
  return author
  """,language="python")
  st.markdown("Thanks to this code, we were able to know the author of an article and compute how many texts he had written over the two years.")
...
def predict():
  ...
  we use _LocalOutlierFactor_ library, allowing us to detect outliers by measuring the local deviation of the density of a given sample with respect to its neighbours 
  ...
  Plus, we use a RobustScaler from _sklearn.preprocessing_ library taking into account InterQuartile Range instead of MinMax Range to scale our variables that are not 
  ...
  intro_logreg ="""Our baseline model for this study will be the **Logistic Regression**. The logistic regression is a classification algorithm from _sklearn.linear_model_ that computes the probability that
  ...
  col3.metric("Accuracy",str(accuracy_dl),str(round(taux(accuracy_dl,0.65),2))+"%")
  ...
  col7.metric("Execution Time",str(exec_dl)+"s",str(round(taux(exec_dl,4.4),2))+"%")
  st.caption("""
  Every result is compared to the base model.
  """)
  st.markdown("""
              The Neural Network is not our most performant model but in our opinion it is the one with the most potential. Here, it is a very scholar model that can be improved heavily by
              the latest research.
              """)
  ...
  comparison_dict = {'Accuracy':(0.67, 0.65, 0.66,0.65, 0.66),
                   ...
                   'Exec Time (sec)':(1.2, 4.4,37.4, 144.1, 295)}
...
def model_template(intro,image_lien,code_algo,accuracy,precision,recall,f1,exec):
  ...
  """)
  col3,col4,col5,col6,col7 = st.columns(5)
  col3.metric("Accuracy",str(accuracy),str(round(taux(accuracy,0.65),2))+"%")
  ...
  col7.metric("Execution Time",str(exec)+"s",str(round(taux(exec,4.4),2))+"%")
  st.caption("""
  Every result is compared to the base model.
  """)
